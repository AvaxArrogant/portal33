...
export async function POST(req: NextRequest) {
  ...
  const name = String(form.get("name") || `${first_name} ${last_name}`);
  const address = String(form.get("address") || "");
  const dob = String(form.get("dob") || "");

  if (!email || !password || !(first_name && last_name)) {
    return NextResponse.json({ error: "email, password, first name, and last name required" }, { status: 400 });
  }
  ...
  const { data, error: createErr } = await admin.auth.admin.createUser({
    ...
    password,
    email_confirm: true,
  user_metadata: { role, name, first_name, last_name, status: "pending", address, dob, created_by: me.id },
  });
